...
const useEngine = () => {
  ...
  // Как только пользователь начинает вводить первую букву, мы начинаем
  useEffect(() => {
    if (isStarting) {
      setState("run");
      startCountdown();
  ...
  // Когда время истекло, мы закончили
  useEffect(() => {
  ...
  /**
   * Когда все текущие слова заполнены,
   * мы генерируем и показываем другой набор слов
   */
  useEffect(() => {
    if (areWordsFinished) {
